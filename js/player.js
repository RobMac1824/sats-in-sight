...
export function updatePlayer(dt) {
  ...
  const hasKeyboardInput =
        state.keyboardInput.x !== 0 || state.keyboardInput.y !== 0;
  ...
  if (state.isTouching) {
        ...
        desiredVx = state.keyboardInput.x * maxSpeed;
        desiredVy = state.keyboardInput.y * maxSpeed;
  }

  state.player.vx += (desiredVx - state.player.vx) * accel;
    state.player.vy += (desiredVy - state.player.vy) * accel;

  if (!state.isTouching && !hasKeyboardInput) {
  ...
  const inset = state.game.hudInset || {
        top: 0,
        right: 0,
        bottom: 0,
        left: 0,
