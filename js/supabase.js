...
export async function fetchLeaderboard() {
    ...
    try {
        ...
        log(
            "ERROR",
            "fetchLeaderboard failed, using localStorage fallback",
            err,
        );
        return {
            scores: sortScores(getLocalScores()),
            fallback: true,
            error: err.message,
        };
    }
...
export async function fetchUserBest(username) {
    ...
    try {
        ...
        if (error) throw error;
        return {
            best: data && data.length ? data[0].score : 0,
            fallback: false,
        };
    } catch (err) {
        log(
            "ERROR",
            "fetchUserBest failed, using localStorage fallback",
            err,
        );
        const scores = getLocalScores();
        const entry = scores.find((item) => item.username === username);
        return {
            best: entry ? entry.score : 0,
            fallback: true,
            error: err.message,
        };
    }
